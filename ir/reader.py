...
def tokenize_cons(token: str, offset: int, stream: Stream) -> ALVMObject:
    if token == ")":
...
def tokenize_int(token: str, offset: int) -> Optional[ALVMObject]:
    try:
...
def tokenize_hex(token: str, offset: int) -> Optional[ALVMObject]:
    if token[:2].upper() == "0X":
        try:
            token = token[2:]
            if len(token) % 2 == 1:
