...
from alvm import to_sexp_f, KEYWORD_FROM_ATOM, KEYWORD_TO_ATOM, SExp
from alvm.EvalError import EvalError
from alvm.serialize import sexp_from_stream, sexp_to_stream
from alvm.operators import OP_REWRITE
...
try:
    from alvm_rs import deserialize_and_run_program, STRICT_MODE
except ImportError:
...
def call_tool(tool_name, desc, conversion, input_args):
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=path_or_code,
        help="path to alvm script, or literal script",
    )
...
def opc(args=sys.argv):
    ...
    call_tool("opc", "Compile a alvm script.", conversion, args)
...
def opd(args=sys.argv):
    ...
    call_tool("opd", "Disassemble a compiled alvm script from hex.", conversion, args)
...
def launch_tool(args, tool_name, default_stage=0):
    ...
    parser = argparse.ArgumentParser(
        description='Execute a alvm script.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="filepath to alvm script, or a literal script")

    parser.add_argument(
        "env", nargs="?", type=path_or_code,
        help="alvm script environment, as alvm src, or hex")
